...
async function tranceSquare(velocity, distance){
...
async function LightCheck(){
	let lightLevel = await getAmbientLight;
    await setDisplayText(`light:${light}`, {r: 255, g: 255, b: 255}, {r: 0, g: 0, b: 0}, 0);
}
...
async function travel(){
	let distance = 5;
	await rollToDistance(0, 10, distance);
	await delay(30);
	await rollToDistance(180, 10, distance);
	
	totalTravel = distance + distance;
	return totalTravel;
}